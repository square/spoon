 ...
 * - implement get()
 *
 * Monte Creasor - July 17, 2017
 * Added UTF-8 string pool handling for Android Stuiod 3+ builds.
 */
public class StringBlock {
    /**
     * String pool is encoded in UTF-8 (otherwise it'm_stringsPool UTF-16).
     */
    private static final int UTF8_FLAG = 1 << 8;
    private static final int CHUNK_TYPE = 0x001C0001;
    private int[] m_stringOffsets;
    /**
     * Pool entries included a string length followed by the string (null terminated).
     */
    private byte[] m_stringsPool;
    /**
     * Offsets into the string pool.
     */
    private int[] m_styleOffsets;
    private int[] m_styles;
    /**
     * Flag indicating whether the string pool contains UTF-8 strings (otherwise they are UTF-16).
     */
    private boolean m_isUtf8;
    /**
     * All strings extracted from the string pool.
     */
    ...
    public static StringBlock read(IntReader reader) throws IOException {
        ...
        }
        {
            ...
            block.m_stringsPool = reader.readByteArray(size);
        }
        if (stylesOffset != 0) {
        ...
        int length = block.m_stringOffsets.length;
        block.m_strings = new String[length];
     ...
     */
    private static int decodeLengthUtf8(ByteBuffer buffer) {
     ...
     */
    public int getCount() {
        return m_strings != null ? m_strings.length : 0;
    }
    ...
    String stringAt(int index) {
        ...
        ByteBuffer buffer =
                ByteBuffer.wrap(m_stringsPool, offset, m_stringsPool.length - offset)
